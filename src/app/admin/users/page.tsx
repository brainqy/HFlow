...
const userRoles: UserRole[] = ['Patient', 'Doctor', 'Nurse', 'Receptionist', 'Admin'];

const userFormSchema = z.object({
  name: z.string().min(2, "Name must be at least 2 characters."),
  email: z.string().email("Invalid email address."),
  role: z.enum(userRoles as [UserRole, ...UserRole[]], { required_error: "Role is required."}), // Ensure zod knows this is a non-empty array
  password: z.string().min(8, "Password must be at least 8 characters.").optional(), // Optional for edit
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    const initialUsers: ManagedUser[] = [
      ...
      { id: 'admin-1', name: 'Admin User', role: 'Admin', email: 'admin@healthflow.clinic', status: 'Active', lastLogin: new Date().toLocaleDateString() }
    ];
  ...
  const onAddUserSubmit = (data: UserFormValues) => {
    const newUser: ManagedUser = {
      id: `${data.role.toLowerCase()}-${new Date().getTime()}`, // Simple unique ID
      name: data.name,
      email: data.email,
      role: data.role,
      status: 'Active',
      lastLogin: 'Never',
      // password: data.password // Don't store password in frontend state
    };
  ...
  const getRoleBadgeVariant = (role: UserRole) => {
    switch(role) {
        ...
        case 'Nurse': return 'default'; // Default primary color
        case 'Receptionist': return 'outline'; // Using outline, can add custom class if needed
