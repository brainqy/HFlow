...
export default function AdminDashboardPage() {
  ...
  const { totalPatients, totalDoctors, appointments } = useMemo(() => {
    ...
    if (dateRange?.from) {
      ...
      return {
        totalPatients: {
          count: Math.floor(allPatients / 5) + Math.floor(Math.random() * 5), // Simulated new patients
          label: `New Patients (${rangeLabel})`,
        },
        totalDoctors: {
          count: allDoctors, // Total doctors usually doesn't change rapidly with date range for dashboard
          label: "Total Doctors",
        },
        appointments: {
          count: Math.floor(allAppointments / 3) + Math.floor(Math.random() * 10), // Simulated count
    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Overview for {todayFormatted}. Welcome, {adminName}!</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <Card className="shadow-md">
          ...
          <CardContent>
            ...
            {!dateRange && <Link href="/admin/users?role=patient" className="text-xs text-primary hover:underline">View Patients</Link>}
          </CardContent>
        </Card>
        <Card className="shadow-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <Link href="/admin/users?role=doctor" className="text-xs text-primary hover:underline">View Doctors</Link>
          </CardContent>
        </Card>
        <Card className="shadow-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-3">
            <Button variant="default" className="w-full" asChild><Link href="/admin/users">Manage Users</Link></Button>
            <Button variant="outline" className="w-full" asChild><Link href="/admin/settings">Manage Clinic Settings</Link></Button>
            <Button variant="outline" className="w-full" asChild><Link href="/admin/reports">View Reports</Link></Button>
          </CardContent>
        </Card>
      </div>
    </div>
